...
Download_brook(){
    ...
    if [[ ${bit} == "x86_64" ]]; then
        ...
        wget --no-check-certificate -N "http://sh.nekoneko.cloud/brook-pf/brook_linux_amd64" -O brook
    else		
		echo -e "${Info} 默认安装v20200801版"
        wget --no-check-certificate -N "http://sh.nekoneko.cloud/brook-pf/brook_linux_386" -O brook
    fi
...
Service_brook(){
    if [[ ${release} = "centos" ]]; then
        if ! wget --no-check-certificate http://sh.nekoneko.cloud/brook-pf/brook-pf_centos -O /etc/init.d/brook-pf; then
        ...
        if ! wget --no-check-certificate http://sh.nekoneko.cloud/brook-pf/brook-pf_debian -O /etc/init.d/brook-pf; then
            echo -e "${Error} Brook服务 管理脚本下载失败 !" && exit 1
        fi
        chmod +x /etc/init.d/brook-pf
